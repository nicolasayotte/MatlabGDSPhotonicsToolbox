function [gelm] = and(gelm1, gelm2)
%function [gelm] = and(gelm1, gelm2)
%
% Defines the '&' operator for the gds_element class, which 
% performs a Boolean 'and' (intersection) operation with the polygons
...
% Ulf Griesmann, NIST, April 2014
   ...
   if ~strcmp(get_etype(gelm1.data.internal), 'boundary') || ...
      ...
      error('gds_element.and :  arguments of & must be boundary elements.');
   end
   ...
   [gelm.data.xy, hf] = poly_boolmex(gelm1.data.xy, gelm2.data.xy, ...
                                     'and', udf);
   if any(hf)
     warning('gds_element.and :  a polygon with a hole was created.');
   end
