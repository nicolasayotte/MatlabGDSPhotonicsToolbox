function [ocoupler] = CouplerGratingSW(type, width, periods, indices, layer, dtype, varargin)
...
[type, width, periods, indices, layer, dtype] = NumberOfRows(rows, type, width, periods, indices, layer, dtype);

ocoupler(rows, 1) = struct('xy', [], 'layer', [], 'dtype', [], 'pos', [], 'ori', 0, 'len', [], ...
  'periods', [], 'indices', [], 'width', [], 'type', '');
...
for ii = 1 : length(ocoupler.indices{col})
  ...
  [ty1, ty2] = LinearDistribution(index, ocoupler.width(col));
...
function [y1, y2] =  LinearDistribution(index, width)
SPACE_MIN = 60e-3;
SPACE_MAX = 1550e-3 / 2;
LINE_MIN = 60e-3;
LINE_MAX = 1550e-3 / 2 / 2.9;

effectiveIndex = linspace(1, 2.9, 20);
refractiveIndex = linspace(1, 3.47, 20);

rindex = interp1(effectiveIndex, refractiveIndex, index);
fillFactor = (rindex - 1) / 2.47;
